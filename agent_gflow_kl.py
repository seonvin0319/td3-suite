...
# === add this util near the top of agent_gflow.py ===
def kl_diag_gaussians(mean_p, logstd_p, mean_q, logstd_q):
    """
    D_KL( N(mean_p, diag(std_p^2)) || N(mean_q, diag(std_q^2)) )
    mean_*, logstd_*: (B, A)
    returns: (B,) KL per sample
    """
    var_p = torch.exp(2.0 * logstd_p)  # std^2
    var_q = torch.exp(2.0 * logstd_q)
    # elementwise: log(sigma_q^2 / sigma_p^2)
    term1 = (2.0 * (logstd_q - logstd_p)).sum(dim=1)
    # trace term + mean diff
    term2 = ((var_p + (mean_p - mean_q)**2) / (var_q + 1e-8)).sum(dim=1)
    k = mean_p.shape[1]
    return 0.5 * (term1 + term2 - k)


class TD3_GFlow_KL_Agent:
    """
    TD3+BC를 gradient flow 관점으로 확장:
      - BehaviorPolicy(가우시안) 회귀
      - Actor(가우시안)과 Behavior 간 symmetric kl divergence penalty
      - Actor 엔트로피 보상
    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            kl_ab = kl_diag_gaussians(mean_a, log_std_a, mean_b, log_std_b)
            kl_ba = kl_diag_gaussians(mean_b, log_std_b, mean_a, log_std_a)
            sym_kl = kl_ab.mean() + kl_ba.mean()

            entropy = 0.5 * (2 * log_std_a + np.log(2 * np.pi * np.e)).sum(dim=1).mean()

            actor_loss = self.kl_weight * sym_kl - lmbda * Q.mean() - self.entropy_weight * entropy
